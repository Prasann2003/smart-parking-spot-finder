...
public interface BookingRepository extends JpaRepository<Booking, Long>,
        ...
        @org.springframework.data.jpa.repository.Query("UPDATE Booking b SET b.status = 'ACTIVE' WHERE b.status = 'CONFIRMED' AND b.startTime <= :now AND b.endTime >= :now")
        int updateActiveBookings(@Param("now") LocalDateTime now);

        @org.springframework.data.jpa.repository.Modifying
        @org.springframework.data.jpa.repository.Query("UPDATE Booking b SET b.status = 'COMPLETED' WHERE b.status IN ('CONFIRMED', 'ACTIVE') AND b.endTime < :now")
